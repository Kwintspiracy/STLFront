    ...
    <div className="min-h-screen bg-[#0F1213] text-white flex flex-col lg:flex-row relative">
      ...
      <div className={`fixed top-0 left-0 z-50 h-full w-64 bg-[#1A1C21] border-r border-[#2A2D30] p-6 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:hidden`}>
        ...
        <nav className="flex flex-col gap-4 text-sm">
          <Link href={`/studio/${studio.id}`} onClick={() => setIsMenuOpen(false)} className="hover:text-primary">Dashboard</Link>
          <Link href={`/studio/${studio.id}/products`} onClick={() => setIsMenuOpen(false)} className="hover:text-primary">My Products</Link>
          <Link href={`/studio/${studio.id}/settings`} onClick={() => setIsMenuOpen(false)} className="hover:text-primary">Settings</Link>
        </nav>
      </div>
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#1A1C21] border-r border-[#2A2D30] p-6 gap-6">
        ...
        <nav className="flex flex-col gap-3 pt-4 text-sm">
          <Link href={`/studio/${studio.id}`} className="hover:text-primary">Dashboard</Link>
          <Link href={`/studio/${studio.id}/products`} className="hover:text-primary">My Products</Link>
          <Link href={`/studio/${studio.id}/settings`} className="hover:text-primary">Settings</Link>
        </nav>
      </aside>
